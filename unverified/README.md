...
| Endgame provenance page | Endgame | [Provenance page](./endgame_provenance_page.md) | which primary sentence names each slot operand: A operand named (colour cells) but no operation; B matrix ambiguous; C boundary pinned to text end, frozen to 3 byte strings; D is an output; only genuine retrieval hole is the lost uninvertible S0 |
| Drive working files 2026-04/05 | SalPhaseIon / Cosmic | [Drive artifacts](./drive-2026-05/README.md) | 4 copies of known blobs; two 1327-byte 'cosmic decryptions' are 1-byte-pad noise; the 'hidden Salted__ blob' has a 1153-byte body, not block-aligned, source missing |
